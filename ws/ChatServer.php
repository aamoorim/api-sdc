        ...
        try {
            ...
            switch ($type) {
                ...
                case 'msg':
                    if (!$user->auth) {
                        $this->send($user, json_encode(['erro' => 'Necessário autenticação']));
                        $this->log("🚫 msg rejeitada: usuário não autenticado (Usuário ID: {$user->id})", 'WARN');
                        return;
                    }

                    if (!$user->current_chamado) {
                        $this->send($user, json_encode(['erro' => 'Usuário não está em nenhum chamado']));
                        $this->log("🚫 msg rejeitada: usuário sem chamado ativo (Usuário ID: {$user->id})", 'WARN');
                        return;
                    }
                    ...
                    if ($texto === '') {
                        ...
                        $this->log("⚠️ Mensagem vazia ignorada (Usuário ID: {$user->id})", 'WARN');
                        return;
                    }

                    $this->log("💾 Gravando mensagem no banco (Usuário {$user->auth['id']}, Chamado {$user->current_chamado}): '{$texto}'", 'DB');
                    ...
                    foreach ($this->users as $u) {
                        if ($u->current_chamado === $user->current_chamado) {
                            ...
                            $this->log("📤 Mensagem enviada (De: {$user->auth['id']} → Para: {$u->id}, Chamado: {$user->current_chamado}) | Conteúdo: '{$texto}'", 'INFO');
                        }
                    }
                    break;
